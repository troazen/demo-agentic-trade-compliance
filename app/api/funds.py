"""
Fund API endpoints with Flask-RESTX for Swagger documentation.
"""
...
# Create namespace for funds
funds_ns = Namespace('funds', description = 'Fund management operations')


@funds_ns.route('/')
class FundsList(Resource):
    @funds_ns.doc('get_all_funds')
    @funds_ns.marshal_with(funds_list_response)
    def get(self):
...
@funds_ns.route('/<int:fund_id>')
class FundDetail(Resource):
    @funds_ns.doc('get_fund')
    @funds_ns.marshal_with(fund_response)
...
@funds_ns.route('/<int:fund_id>/cash')
class FundCashUpdate(Resource):
    @funds_ns.doc('update_fund_cash')
    @funds_ns.expect(fund_cash_update_request)
...
@funds_ns.route('/<int:fund_id>/total-assets')
class FundTotalAssets(Resource):
    @funds_ns.doc('get_fund_total_assets')
    @funds_ns.marshal_with(total_assets_response)
    def get(self, fund_id):
...
@funds_ns.route('/<int:fund_id>/net-assets')
class FundNetAssets(Resource):
    @funds_ns.doc('get_fund_net_assets')
    @funds_ns.marshal_with(net_assets_response)
    def get(self, fund_id):
        """Get fund net assets."""
...
@funds_ns.route('/<int:fund_id>/total-assets-ex-cash')
class FundTotalAssetsExCash(Resource):
    @funds_ns.doc('get_fund_total_assets_ex_cash')
    @funds_ns.marshal_with(total_assets_ex_cash_response)
    def get(self, fund_id):
...
@funds_ns.route('/')
class FundCreate(Resource):
    @funds_ns.doc('create_fund')
    @funds_ns.expect(fund_create_request)
    @funds_ns.marshal_with(success_response)
    def post(self):
        """Create a new fund."""
