        ...
        if selected_alert_id:
            try:
                ...
                if not alert_details:
                    st.warning(f"Alert details not found for alert ID '{selected_alert_id}'. The alert may not exist in the database or may be missing required data.")
                else:
                    col1, col2 = st.columns(2)
                    
                    with col1:
                        st.markdown(f"**Alert ID:** {alert_details.get('alert_id', 'N/A')}")
